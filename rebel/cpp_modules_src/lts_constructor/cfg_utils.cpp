...
{
  return visited_blocks.find(block) != visited_blocks.end();
}


bool isProcessed(std::set<llvm::BasicBlock *> &processed_blocks, llvm::BasicBlock *block)
{
  return processed_blocks.find(block) != processed_blocks.end();
}
